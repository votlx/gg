    ...
    if replied:
        if replied.video or replied.document:
            ...
            if len(m.command) < 2:
                ...
                if pq == "360" or "360" or "360":
                    Q = int(pq)
                else:
                    Q = 360
                    await loser.edit(
                        "» __فقط 360, 360, 360 مسموح__ \n💡 ** الان يشتغل الفيديو في 360p**"
                    )
            try:
                if replied.video:
...
async def vstream(c: Client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if len(m.command) == 2:
            ...
            await m.reply("**/vstream {link} {360/360/360}**")

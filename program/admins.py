...
@Client.on_message(command(["skip", f"تخطي", "تخطى"]) & other_filters)
@authorized_users_only
...
@Client.on_message(
    command(["ايقاف", f"stop@{BOT_USERNAME}", "end", f"نهاء", "انهاء"])
    & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["مستوى", f"volume@{BOT_USERNAME}", "تحكم"]) & other_filters
)
@authorized_users_only
